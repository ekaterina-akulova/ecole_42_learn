...
ClapTrap::ClapTrap() {
    std::cout << "Default CP constructor called" << std::endl;
};


ClapTrap::ClapTrap(std::string name) : name(name), Hit_points(10), \
Energy_points(10), Attack_damage(0) {
    std::cout << "Constructor  for " << this->name << " called (CP)" <<  std::endl;
}

ClapTrap::~ClapTrap() {
    std::cout << "Destructor CP called for " << this->name << std::endl;
}
...
Attack_damage(object.Attack_damage) {
    std::cout << this->name << " copy CT constructor called" << std::endl;
}
