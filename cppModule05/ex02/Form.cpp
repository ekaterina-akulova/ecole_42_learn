...
void Form::execute(Bureacrat &executor) const {
    try
    {
    ...
    {
        lexp.display();
        std::cout << executor.getName() << " couldn’t execute " << this->name << "because GradeTooLow" << std::endl;
    }
    catch (const char *str) {
        std::cerr	<< name << " couldn’t execute " << this->getName()
